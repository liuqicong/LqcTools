...
public class QRCodeEncoder {
    ...
    /**
     * 同步创建指定前景色、白色背景色、带logo的二维码图片。该方法是耗时操作，请在子线程中调用。去掉白边
     *
     * @param content         要生成的二维码图片内容
     * @param size            图片宽高，单位为px
     * @param foregroundColor 二维码图片的前景色
     * @param logo            二维码图片的logo
     */
    public static Bitmap syncEncodeQRCodeDeleteWhite(String content, int size, int foregroundColor, Bitmap logo) {
        return syncEncodeQRCodeDeleteWhite(content, size, foregroundColor, Color.WHITE, logo);
    }
    ...
    public static Bitmap syncEncodeQRCode(String content, int size, int foregroundColor, int backgroundColor, Bitmap logo) {
        try {
            //临时解决问题 二维码留白问题
            // 当二维码宽度太小，源码中会认为不合理，所以会取一个合适的值，会造成留白
            // 修改源码 在QRCodeWriter类调用renderResult之前重新设置width和height，将留白控制在某个数值以内
//            if(size < 160) {
//                size = 160;
//            }
...
//            matrix = deleteWhite(matrix);//删除白边
     ...
     * @param logo            二维码图片的logo
     */
    public static Bitmap syncEncodeQRCodeDeleteWhite(String content, int size, int foregroundColor, int backgroundColor, Bitmap logo) {
        try {
            BitMatrix matrix = new MultiFormatWriter().encode(content, BarcodeFormat.QR_CODE, size, size, HINTS);
            /**由于google zxing 源码会对二维码进行缩放，导致会出现白边
             * 在encoder的时候按实际尺寸成倍数缩放
             * 当倍数* input.getWidth() 值小于size的时候
             * 多余的为空白区域
             * 解决方案：判断前后的空白区域大小，用isWhiteSpace判断出初始空白区域，获取到空白宽度
             * 在通过空白空度，去掉其他方向的空白区域
             */
            int[] pixels = new int[size * size];
            ...
            for (int y = 0; y < size; y++) {
                for (int x = 0; x < size; x++) {
                    if (matrix.get(x, y)) {
                        ...
                        //判断非空白区域
                        if(!isWhiteSpace && x >= whiteLength && y >= whiteLength && (size - x) >= whiteLength  && (size - y) >= whiteLength) {
                            pixels[y * size + x] = backgroundColor;
                        }
                    }
                }
